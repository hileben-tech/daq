...
#if 1
static volatile unsigned long *ecap0_in_pwm0_out = NULL;
static volatile unsigned long *mcasp0_aclkr = NULL;

#define ECAP0_IN_PWM0_OUT	0x44e10964	//GPIO0_7
#define MCASP0_ACLKR		0x44e109a0	//GPIO3_18
#endif
static void serial_omap_stop_tx(struct uart_port *port)
{
	...
	pm_runtime_put_autosuspend(up->dev);
	wait_for_xmitr(up);
//printk("===========================1:0x%x %d\n", up->rs485.flags, up->rs485.gpio_pin);
...
#if 0
	ecap0_in_pwm0_out = ioremap(ECAP0_IN_PWM0_OUT, 0x2);
	mcasp0_aclkr = ioremap(MCASP0_ACLKR, 0x2);
#endif
	pm_runtime_mark_last_busy(up->dev);
	pm_runtime_put_autosuspend(up->dev);
	up->port_activity = jiffies;
...
serial_omap_config_rs485(struct uart_port *port, struct serial_rs485 *rs485conf)
{
	...
	if (rs485conf->flags & SER_RS485_ENABLED) {
		...
		printk("%s: 0 - 0x%x %d\n", __func__, rs485conf->flags, rs485conf->gpio_pin);
		printk("%s: 1 - 0x%x %d\n", __func__, p->rs485.flags, p->rs485.gpio_pin);
...
serial_omap_ioctl(struct uart_port *port, unsigned int cmd, unsigned long arg)
{
	...
	case TIOCSRS485:
		...
		if (copy_from_user(&rs485conf, (struct serial_rs485 *)arg,
			        sizeof(rs485conf)))
			return -EFAULT;
#if 1  // DEFINE_RS485_GPIO
		rs485conf.flags |= SER_RS485_USE_GPIO;
		rs485conf.gpio_pin = 7; // GPIO0_7
		printk("[%s] rs485conf.flags:0x%x %d\n", __func__, rs485conf.flags, SER_RS485_USE_GPIO);
#endif
...
{
#if 0
	iounmap(ecap0_in_pwm0_out);
	iounmap(mcasp0_aclkr);
#endif
